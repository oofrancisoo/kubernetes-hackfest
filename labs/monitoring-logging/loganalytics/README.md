...
3. Verify you are seeing logs in OMS by starting a container and sending logs to STDOUT.
...
When you are exec'd into the contatiner run the following command to generate logs to STDOUT. 

   ```bash
   echo "hackfest test logs " >> /proc/1/fd/1
    ```
Give this a few minutes to appears in Log Analytics. Run the following to see the logs. 

   ```bash
   Open the Azure Portal | Select the Kubernetes service object in the Azure Portal | On the Kubernetes service object | Select the Insights (preview) | Select Nodes | drill down through each of the nodes and find the nginx pod | after you select the pod choose the Logs link and this will open Log Analytics. Look through the logs until you see the output from above. 
    ```
